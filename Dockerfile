...
#RUN mkdir -p /go/src/github.com/holdskill/ && cd /go/src/github.com/holdskill/ && git clone https://github.com/holdskill/docsystem.git && cd docsystem

ADD . /go/src/github.com/holdskill/docsystem

WORKDIR /go/src/github.com/holdskill/docsystem

RUN	 go get -u github.com/golang/dep/cmd/dep && dep ensure  && \
	CGO_ENABLE=1 go build -v -a -o docsystem_linux_amd64 -ldflags="-w -s -X main.VERSION=$TAG -X 'main.BUILD_TIME=`date`' -X 'main.GO_VERSION=`go version`'" && \
...
ADD start.sh /go/src/github.com/holdskill/docsystem
...
COPY --from=build /go/src/github.com/holdskill/docsystem /docsystem
